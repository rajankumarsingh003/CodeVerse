...
//       try {
//         const res = await axios.get(`${API}/getHistory/${encodeURIComponent(username)}`);
//         setHistory(res.data);
//       } catch (err) {
...
//         ) : (
//           history.map((item) => (
//             <div key={item._id} className="border-b border-gray-700 pb-3">
//               <p className="font-semibold text-blue-400 mb-1">Q: {item.question}</p>
...
// down code is of professionalcode
...
const HistoryModal = ({ username, onClose }) => {
  ...
  const clearHistory = async () => {
    ...
    setClearing(true);
    try {
      await axios.delete(`${API}/clearHistory/${encodeURIComponent(username)}`);
      setHistory([]);
    } catch (err) {
